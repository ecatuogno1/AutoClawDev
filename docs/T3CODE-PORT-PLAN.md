...
T3Code: `~/Developer/t3code`
**IMPORTANT: Use branch `personal/daily-driver`** — this has the workspace/IDE features.
The `main` branch does NOT have the workspace system.

Before starting any phase, run:
```bash
cd ~/Developer/t3code && git checkout personal/daily-driver
```

Key commit: `bb309774 feat: workspace tab system, browser views, activity bar, and compositor pane layout`
— 176 files changed, +25,353 lines. This is the workspace implementation to port.
...
### Server (`apps/server/src/`) — on `personal/daily-driver`
...
- **Workspace** (`workspace/`) — workspace command service, snapshot queries, projection, project management (NEW on daily-driver)
...
### Web (`apps/web/src/`) — on `personal/daily-driver`
- **Routes**: `_chat.tsx` (main layout), `_chat.$threadId.tsx` (thread view), `_chat.settings.tsx`, `_chat.workspaces.$workspaceId.tsx` (workspace view — NEW on daily-driver)
- **Components**:
